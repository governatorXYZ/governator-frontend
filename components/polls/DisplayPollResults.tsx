...
const getCountdown = (pollData: Poll) => {
  ...
  const days = Math.floor(deltaT_ms / MS_PER_DAY)
    ? Math.floor(deltaT_ms / MS_PER_DAY)
    : 0
  const hours = Math.floor((deltaT_ms - days * MS_PER_DAY) / MS_PER_HOUR)
    ? Math.floor((deltaT_ms - days * MS_PER_DAY) / MS_PER_HOUR)
    : 0
  const minutes = Math.floor(
    (deltaT_ms - (hours * MS_PER_HOUR + days * MS_PER_DAY)) / MS_PER_MINUTE
  )
    ? Math.floor(
        (deltaT_ms - (hours * MS_PER_HOUR + days * MS_PER_DAY)) / MS_PER_MINUTE
      )
  ...
  const seconds = Math.floor(
    (deltaT_ms -
      (minutes * MS_PER_MINUTE + hours * MS_PER_HOUR + days * MS_PER_DAY)) /
      MS_PER_SECOND
  )
    ? Math.floor(
        (deltaT_ms -
...
const Timer: React.FC<DisplayPollResultsProps> = ({
  pollData,
  onCountdownComplete,
}) => {
  ...
  return (
    <Tooltip
      hasArrow
      label={
        new Date(pollData.end_time).toLocaleDateString() +
        ' ' +
        new Date(pollData.end_time).toLocaleTimeString()
      }
      bg='dark-1'
    >
      <Center>
...
const ShieldBlock: React.FC<DisplayPollResultsProps> = ({
  pollData,
  totalVotes,
  onCountdownComplete,
}) => {
...
const ResultBlock: React.FC<DisplayPollResultsProps> = ({
  pollData,
  voteData,
  totalVotes,
}) => {
    ...
    <div>
      {!voteData.aggregate || voteData.aggregate.length === 0 ? (
        <Flex
          ...
          // maxW='max-content'
        >
          <Box width='100%' backgroundColor='blue'></Box>
          <VStack>
            <Box>
          ...
          <Box width='100%' backgroundColor='yellow'></Box>
        </Flex>
      ) : (
        <div>
          <Flex width={450}>
          ...
          <Box>
            <Card title='Total Votes' value={totalVotes ?? '0'}></Card>
          </Box>
        </div>
      )}
    </div>
...
const PollResultStack: React.FC<DisplayPollResultsProps> = ({
  pollData,
  result,
}) => {
  ...
  if (pollData.strategy_config && pollData.strategy_config.length > 0) {
    strategyName = strategies.find(
      (strat: { label: string; value: string }) =>
        strat.value === pollData.strategy_config[0].strategy_id
    )?.label
  ...
  return (
    <VStack spacing='16px'>
      ...
      <Stack spacing='4' width='100%' p='5px'>
        <Accordion
          style={{ outline: 'none' }}
          defaultIndex={[0, 1, 2]}
          allowMultiple
        >
            ...
            <h2>
              <AccordionButton pl='0'>
                <Box as='span' flex='1' textAlign='left'>
                  <Heading size='xs' textTransform='uppercase' color='gray.100'>
                    Description
                  </Heading>
                </Box>
          ...
          <AccordionItem style={{ outline: 'none', borderWidth: '0px' }}>
            ...
            <AccordionPanel pb={4}>{result}</AccordionPanel>
          </AccordionItem>
        </Accordion>
      </Stack>
    </VStack>
